...
(async () => {
  ...
  // Unlock WebAudio on first user interaction (required on iOS)
  ...
  // --- Sound effects (Web Audio with GitHub RAW fallbacks) ---
  const AUDIO_URLS = {
    right: [
      "https://github.com/inspiringsource/pixi-quiz/raw/refs/heads/main/yay-soundeffect.mp3",
      "https://raw.githubusercontent.com/inspiringsource/pixi-quiz/refs/heads/main/yay-soundeffect.mp3",
      "/yay-soundeffect.mp3",
      "/public/yay-soundeffect.mp3",
    ],
    wrong: [
      "https://github.com/inspiringsource/pixi-quiz/raw/refs/heads/main/oh-soudeffect.mp3",
      "https://raw.githubusercontent.com/inspiringsource/pixi-quiz/refs/heads/main/oh-soudeffect.mp3",
      "https://github.com/inspiringsource/pixi-quiz/raw/refs/heads/main/oh-soundeffect.mp3",
      "https://raw.githubusercontent.com/inspiringsource/pixi-quiz/refs/heads/main/oh-soundeffect.mp3",
      "/oh-soudeffect.mp3",
      "/oh-soundeffect.mp3",
      "/public/oh-soudeffect.mp3",
      "/public/oh-soundeffect.mp3",
    ],
  };
  let audioCtx = null;
  const audioBuffers = {};
  ...
  async function decodeFirstAvailable(urlList) {
  ...
  async function unlockAudioOnce() {
    ...
    // tiny silent tick to fully unlock on some iOS versions
  ...
  async function playSfx(kind) {
    ...
    if (!audioBuffers[kind]) {
      audioBuffers[kind] = await decodeFirstAvailable(AUDIO_URLS[kind]);
    }
  ...
  const QUESTION_BANK = [
    ...
    {
      q: "Welches Schlüsselwort zeigt Vererbung an?",
      opts: ["with", "extends", "inherits", "include"],
      ans: 1,
    },
    {
      q: "Welcher Datentyp speichert ganze Zahlen?",
      opts: ["float", "int", "boolean", "char"],
      ans: 1,
    },
    {
      q: "Welche Sammlung ist geordnet und erlaubt Duplikate?",
      opts: ["Set", "List", "Map", "Enum"],
      ans: 1,
    },
    {
      q: "Welche Sichtbarkeit erlaubt Zugriff aus überall?",
      opts: ["private", "protected", "package-private", "public"],
      ans: 3,
    },
    {
      q: "Standardwert eines int-Felds (nicht initialisiert)?",
      opts: ["null", "0", "NaN", "undefined"],
      ans: 1,
    },
    {
      q: "Welches Schlüsselwort verbietet Überschreiben einer Methode?",
      opts: ["abstract", "final", "static", "default"],
      ans: 1,
    },
    {
      q: "Welche Schnittstelle wird typischerweise zum Sortieren benutzt?",
      opts: ["Cloneable", "Serializable", "Comparable", "Runnable"],
      ans: 2,
    },
  ];
  ...
  app.stage.addChild(effects); // confetti layer
  const overlay = new Container();
  app.stage.addChild(overlay); // results screen
  overlay.visible = false;

  /* --- Title / Question / Status --- */
  const title = new Text({ text: "Teitelbaum Java Quiz", style: titleStyle });
  root.addChild(title);
  const questionText = new Text({ text: "", style: qStyle });
  ...
  function renderPie(correct, wrong) {
    ...
    const maxRByWidth = Math.floor(
      Math.min(
        90,
        Math.max(60, Math.min(cardW * 0.35, (cardW - pad * 2) / 2 - 10))
      )
    );
    const maxRByHeight = Math.floor(
      (resultCard.y +
        cardH -
        16 -
        48 /*restart btn*/ -
  ...
  const layout = () => {
    ...
    const estRadius = Math.floor(
      Math.min(90, Math.max(60, Math.min(cardW * 0.35, (cardW - 32) / 2 - 10)))
    );
    ...
    restartBtn.position.set(
      resultCard.x + 16,
      Math.round(resultCard.y + cardH - 16 - restartH)
    );
  ...
  function setUI() {
    if (idx >= questions.length) {
      ...
      resultStats.text =
        `Richtig: ${score}   |   Falsch: ${wrong}   |   Gesamt: ${total}` +
        "\n\n";
  ...
  const COLORS = [
    0xff7aa2, 0x7dd3fc, 0x34d399, 0xf59e0b, 0x93c5fd, 0xfda4af, 0xf472b6,
    0xa78bfa,
  ];
